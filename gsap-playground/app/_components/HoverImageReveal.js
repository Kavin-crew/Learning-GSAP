 ...
 * HoverImageReveal
 *
 * A reusable hover-to-reveal image list component, originally built with
 * Framer Motion. Hovering a text row swaps in the associated image, which
 * follows the cursor.
 *
 ...
 *
 * @param {Object} props
 * @param {{itemCount?: number, [key: string]: any}} [props.items] - itemN (1..6) entries: { text, image: { src, srcSet, alt }, link }
 * @param {Object} [props.font] - CSS font-related properties spread onto the container
 * @param {string} [props.textColor]
 * @param {string} [props.dimColor]
 * @param {"left"|"center"|"right"} [props.align]
 * @param {number} [props.rowGap]
 ...
 * @param {number} [props.followStrength]
...
export default function HoverImageReveal({
  ...
  const count = Math.max(1, Math.min(MAX_ITEMS, data.itemCount || 5));
  const list = [];
