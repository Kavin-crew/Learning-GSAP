...
import FlairCursorTrail from "./ui/FlairCursorTrail";
import FlairCursorTrailSection from "./ui/FlairCursorTrailSection";
...
export default function Hero() {
  ...
  // transition from current video to next video when mini video is clicked
  // (zooming in the mini video and zooming out the current video)
  useGSAP(
    () => {
      if (hasClicked) {
  ...
  // animate the clip-path and border-radius of the video frame on scroll
  useGSAP(() => {
  ...
  return (
    <FlairCursorTrailSection
      images={[...flairImages, ...flairImages]}
      gap={80}
      size={50}
    >
      <section className="relative h-dvh w-screen overflow-x-hidden">
        {/* cursor applies entire page */}
        {/* <FlairCursorTrail images={flairImages} gap={100} size={50} /> */}
